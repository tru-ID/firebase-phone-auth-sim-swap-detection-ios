...
As you see, it is a pretty simple project with a single ViewControlller and this is enough to demonstrate SimCheck and Firebase Phone Authentication.
...
- A `UIActivityIndicator` (Large) to show/hide progress when you perform a SIMCheck
...
//TODO Insert /Users/didemyakici/Desktop/Screenshot 2021-05-05 at 13.27.22.png
...
You need to connect the `UITextField` as well. Let's perform the above steps for it as well and name them as `phoneNumberTextField`.
...
//TODO Screenshot NEEEDEDDD

## Add Firebase Phone Authentication
Now that you have your Firebase project set up and your User Interface built up, the next step is to enable signing in with your Phone.  When you click on the Phone within the [Authentication signing-in providers](https://console.firebase.google.com/u/0/project/iosfirebasetruid/authentication/providers), after enabling you will need to follow the instructions for [Authenticate with Firebase on iOS using a Phone Number](https://firebase.google.com/docs/auth/ios/phone-auth?authuser=0)
//TODO Yukaridaki cumle dusuk

You have already completed the **Before you begin** section and **Step 1** Enable Phone Number sign-in for your Firebase project, so proceed to **Step 2** Enable app verification:
...
### [**Send a verification code to the user's phone**](https://firebase.google.com/docs/auth/ios/phone-auth?authuser=0#send-a-verification-code-to-the-users-phone)
...
```
// TODO Yukaridaki codu nereye ekliyoruz???
...
Now, your code should look like below at this point:
```swift
...
###[**Sign in the user with the verification code**](https://firebase.google.com/docs/auth/ios/phone-auth?authuser=0#sign-in-the-user-with-the-verification-code)
...
Now you completed the code for authenticating with Firebase. Next, we need to place the above code into a reusable function called `executeFirebasePhoneVerification()`. 

//TODO declare and tell the developer about this function before. Icini doldurt.
...
For this, we'll need to make an HTTP POST request to the `sim-check` endpoint (ie. localtunnel URL + /sim-check). The url will look like the following: `https://wonderful-lionfish-40.loca.lt/sim-check`. In a production environment you should use your own servers.
...
let url = URL(string: "https://wonderful-lionfish-40.loca.lt/sim-check")!
...
The network request is executed by calling `task.resume()`and the completion handler is onvoked at some point.

//TODO Note to self: Enter the full code at this point
