...
It is time to write some code to manage the UI state. Add the following to `ViewController.swift`:
...
This method helps us show or hide the `busyActivityIndicator`. It also disables the `phoneNumberTextField` and `verifyButton` when the FirebasePhoneAuthentication/SIMCheck flow is in progress. You will use this method later in the `verify(_ sender: Any)` method.
...
To do this, create a method for the Firebase Phone verification in `ViewController.swift` named `executeFirebasePhoneVerification` to be called when the `verify` button is touched.
...
The next step within `verify` is to use the previously created `controls` method by setting the `enabled` variable to `false`. This allows us to disable the phone number text field and the verify button, show the activity indicator, and start spinning it when the user taps the **Verify** button. 
...
Restore the UI controls back to their original state with the following code so that the user can re-execute the workflow:
...
In various cases the controls are re-enabled via `self?.controls(enabled: true)`.
...
If any errors occurred, the HTTP response is not a `200`, or there is a problem deserializing the response, then call the `completionHandler` with `false` and the `error`.
...
Finally, integrate the new `truIDSIMCheckVerification` method so it's executed before the `executeFirebasePhoneVerification` method within the `verify` method.
...
Also see [this Stackoverflow question](https://stackoverflow.com/questions/48727206/firebase-cocoapods-error-message-could-not-build-objective-c-module-firebase/48728282).
